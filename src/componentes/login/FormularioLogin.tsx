...
export function FormularioLogin() {
  ...
  const enviar = form.handleSubmit(async (valores) => {
    ...
    setErrorCaptcha("");
    await loginMutation.mutateAsync({ username: valores.email, password: valores.password });
  ...
  return (
    <form onSubmit={enviar} className="space-y-4">
      ...
      <SelectorIdioma />
      {loginMutation.isError ? <p className="rounded-md bg-red-50 px-3 py-2 text-sm text-red-700">Credenciales incorrectas o usuario inactivo.</p> : null}
