...
export function CmdRecordFilters({
  ...
  return (
    <div className="grid gap-3 rounded border border-slate-200 bg-white p-4 md:grid-cols-3 xl:grid-cols-6">
      ...
      <input value={filters.deviceName ?? ""} onChange={(event) => onChange({ ...filters, deviceName: event.target.value })} placeholder="Device name" className="rounded border border-slate-200 px-3 py-2 text-sm" />
      ...
      <input value={filters.operator ?? ""} onChange={(event) => onChange({ ...filters, operator: event.target.value })} placeholder="Operator/User" className="rounded border border-slate-200 px-3 py-2 text-sm" />
      ...
      <input value={filters.content ?? ""} onChange={(event) => onChange({ ...filters, content: event.target.value })} placeholder="Payload summary" className="rounded border border-slate-200 px-3 py-2 text-sm" />
      <input value={filters.startDate ?? ""} onChange={(event) => onChange({ ...filters, startDate: event.target.value })} type="datetime-local" className="rounded border border-slate-200 px-3 py-2 text-sm" aria-label="Create time start" />
      <input value={filters.endDate ?? ""} onChange={(event) => onChange({ ...filters, endDate: event.target.value })} type="datetime-local" className="rounded border border-slate-200 px-3 py-2 text-sm" aria-label="Create time end" />
    </div>
  );
}
