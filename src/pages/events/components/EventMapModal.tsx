...
export function EventMapModal({ abierto, latitude, longitude, onCerrar }: { abierto: boolean; latitude?: number; longitude?: number; onCerrar: () => void }) {
  ...
  return (
    <Modal abierto={abierto} titulo="Event position" onCerrar={onCerrar}>
      {valid ? (
        ...
        <p className="text-sm text-slate-600">No valid GPS position available for this event.</p>
      )}
    </Modal>
  );
}
