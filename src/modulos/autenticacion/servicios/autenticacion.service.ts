...
export async function iniciarSesion(credenciales: CredencialesLogin) {
  const respuesta = await api.post<RespuestaLogin | { data: RespuestaLogin }>("/api/auth/login", credenciales);
  return extraerDatos<RespuestaLogin>(respuesta);
}

export async function obtenerUsuarioActual() {
  const respuesta = await api.get<UsuarioSesion | { data: UsuarioSesion } | { user: UsuarioSesion }>("/api/auth/me");
